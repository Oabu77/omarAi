...
Android-first, policy-conscious vertical slice for **Omar AI™ — Your AI for life and business**.

## What is implemented

- Four-step onboarding with optional interests, honest integration scope, and contextual-permission explanation.
- Premium Material 3 home using the approved Omar AI icon and the primary prompt: “What do you want Omar AI to do?”
...
- Local JSON export and destructive local-data deletion confirmation. Export reports success only after the selected stream is written. Deletion clears Room tables, onboarding/app preferences, Omar camera-preview cache files, and any legacy persisted URI grants. Android cloud backup is disabled.
- Google Play Billing 9.1 hooks for `omar_ai_pro` and `omar_ai_business`. Prices come only from Play. No entitlement is granted unless the backend verifies the purchase token and returns an active entitlement with `grantsAccess=true` and provider evidence. A pending-activation response never grants access.
...
```properties
OMAR_API_BASE_URL=https://api.example.com/
OMAR_PRIVACY_POLICY_URL=https://example.com/privacy
OMAR_ACCOUNT_DELETION_URL=https://example.com/delete-account
```
...
The release manifest requests only `INTERNET` and `CAMERA`. Camera is requested after the user taps Take photo and sees a purpose explanation. Voice input launches Android's speech-recognition service and does not require this app to request `RECORD_AUDIO`; the resulting text is placed in the input field. Photos and documents use system pickers, so the app requests no broad media/storage permission and does not retain a persistable picker grant. Camera previews use an Omar-prefixed cache file that is removed when discarded/submitted or when the screen closes; local-data deletion also clears any remaining preview files, legacy grants, Room data, and app preferences. `POST_NOTIFICATIONS`, contacts, SMS, call-log, location, background location, exact alarm, and Accessibility permissions are absent.

Before any Play production submission, implement a real authentication flow that populates the explicit in-memory token provider with a verified JWT (and a secure persistence/revocation design if sessions survive restarts), configure real public privacy/deletion URLs, connect and test the backend, create the two Play subscription products, configure backend purchase acknowledgment/RTDN, supply release signing, and complete the actual Data Safety/App Access/financial-feature declarations from the final binary and SDK inventory. Billing purchase launch, integration refresh, remote tasks, and remote deletion remain gated while no authenticated session exists.
